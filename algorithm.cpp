...
static int     make_true(t_expert **head, char *line, char *first, int prev_amb, int g_view)
{
    int i;
    int ret;
        ...
        while (rules[++i])
        {
            if (!break_into_two(rules[i], &first, &last))
            {
                if (check_truth(head, first, 0, 0))
                    t = make_true(head, last, first, prev_amb(first, *head), g_view);
            }
            else
            {
                if (check_truth(head, last, 0, 0))
                    t = make_true(head, first, last, prev_amb(last, *head), g_view);
                if (check_truth(head, first, 0, 0))
                    t = make_true(head, last, first, prev_amb(first, *head), g_view);
            }
            if (first)
